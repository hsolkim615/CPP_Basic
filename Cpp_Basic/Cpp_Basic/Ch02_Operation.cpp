...
void OperationIncrementDecrement() // 증감연산
{
  // 증감 연산
  int Increase = 0; // 초기화 
  int Decrease = 10;
  ...
  Increase++; // Count : 개수 - 1씩 증가, 감소
  Decrease--;
  
  cout << Increase << endl << Decrease << endl;
}

void OperationComparison() // 비교연산
{
...
void OperationLogical() // 논리연산
{
  ...
  cout << !( kor >= 90 && eng >= 90 ) << endl; // ! : 결과 값이 반대가 됨
  cout << ( !(kor >= 90) && eng >= 90 ) << endl;  
}

void OperationBit() // 비트연산
{
  // 비트 연산 
