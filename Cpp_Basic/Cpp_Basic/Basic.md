...
- if 구문의 실행 분기 선택 여부느 if가 다루는 bool type 값(true/false)에 따라서 결정
...
- 구문3 : for loop가 실행될 때 코드블록을 실행한 이후에 실행되는 구문
- 구문 2 : 조건식, 구문 3 : 증감식
...
함수 선언(정의)
void Funtion(para) <= parameter
{

}

함수 호출
Function(10); <= 10 : arguement
...
# 오버 라이딩
  ...
  - 부모 클래스에 함수와 자식 클래스의 함수이름이 동일하게 사용됨으로써 부모클래스의 함수가 숨겨지는 기능
...
### API(Application Programming Interface)
    ...
    - 일종의 소프트웨어 인터페이스이며 다른 종류의 소프트웨어 서비스를 제공한다. 
...
# 가상함수
    ...
    - 부모 클래스에 있는 함수를 자식 클래스에서 오버라이딩 할 것을 예측해서 정의한 함수
